...
public class Ex01 {
	public static void main(String[] args) {
		...
		String msg = "한국 ICT 인재 개발원 1강의장"; // <=위치값
		// 1. charAt(int index) : char
		// index <- 위치값 / 위치값을 받아서 그 위치에 존재하는 문자를 반환한다.
		// 이때 위치값(index)은 0부터이다.
		char c1 = msg.charAt(7); // 인
		System.out.println(c1);

		// 문제) 다음 문자들 중에 소문자를 대문자로 변경 하시오.
		// 힌트) a는97, A는 65이다. (차이는32)
		...
		// 받은 문자열과 현재 문자열이 같으면 true, 다르면 false (대소문자 구별안함)
		// ** 주의) 문자열이나 객체를 비교할때 '==' 는 사용하면 내용이 같냐? 아니라 주소가 같냐?
		...
		// 11. indexOf(String) : int
		// 문자열을 받아서 해당 문자열의 위치값을 표시
		// 시작위치는 처음부터, 찾는 문자가 없으면 -1이다.

		// 13. indexOf(int str, int fromIndex) : int
		...
		// 	   replace(CharSequence target, String replacement) : String
		//	   문자나 문자열을 받아서 치환하기
		String msg13 = "대한민국";
		...
		//	   구분자를 받아서 배열로 나눈다. (limit가 배열의 크기이다)
		...
		//System.out.println(res[1]);
			...
			String[] res3 = msg17.split("," , 10);
					...
					//	   substring(int beginIndex, int endInsdex) : String
					...
					// 태어난 달 추출
					String res8 = msg19.substring(0, 4);
					System.out.println("=====================================");
					...
					// 각종자료형에 +1;
					//System.out.println(p1+1);  / boolean은 + 연산을 할수 없다.
					...
					// 21의 반대 개념 숫자 모양의 문자열을 숫자로 변경(각종 자료형 모양의 문자열을 각종 자료형으로 변경)
					// Wrapper class ( Boolean, Integer, Double, 
					//
					// - 각 자료형의 형태를 가진 문자열을 진짜 자료형으로 변경 시키는 클래스들
					// - 원래 의미는 기본자료형을 객체로 만들때 사용 하는 클래스
					//	 기본자료형 => 객체형 자동으로 변경 (오토박싱)
					// 	 객체형 => 기본자료형 자동으로 변경 (오토언박싱)
					
					// 1. boolean형태의 문자열을 Boolean 형으로 변경
					//	  Boolean.parseBoolean("true"); Boolean.parseBoolean("나머지글자");
					msg = "true";
					...
					// (gender가 1 또는 2 면 1900년대 생, 3또는 4 면 2000년대 생)
