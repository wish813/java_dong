...
public class Ex01 {
	public static void main(String[] args) {
		...
		//											Runnable target인터페이스를 인자로 받아서 처리
		// - thread(String name) : 이때 name은 스레드이름 지정할 수 있다.
		
		// 주요메소드
		// - join() : 현재 스레드는 join() 메소드를 호출한 스레드가 끝날때까지
		//			  대기상태로 빠져있다가 join()를 호출한 스레드가 끝나야 다시 실행한다.
		// 			  즉, join()를 호출한 스레드가 끝날때 까지 실행할 수 없다.
		// - start() : start()를 실행하면 run()를 호출한다.(만약 start()가 있으면 start()로 간다.)
		...
		// - (현재라는뜻)curruntThread() : 현재 수행되는 스레드 객체를 리턴한다.(static)
